...
for (i in 1:length(models)) {
  ...
  classPos <- suppressWarnings(which(
    (substr(docu, start = 1, stop = 6) == "Class ") &
      (grepl(" [-–] ", docu)) &
  ...
  if (length(endPos) < length(descriptionPos)) {
    maxPosClass <- c(
      classPos[-1],
      min(
        which(substr(docu, start = 1, stop = 18) == "Model Parameters"),
        which(substr(docu, start = 1, stop = 10) == "References")
      )
    )
    endPos <- vector()
    for (k in 1:length(descriptionPos)) {
      mp <- ifelse(
        (k == length(descriptionPos)),
        maxPosClass[k],
        descriptionPos[k + 1]
      )
      maxPosTree <- which(
        substr(docu[descriptionPos[k]:mp], start = 1, stop = 12) ==
          "Maximum Tree"
      ) +
        descriptionPos[k]
  ...
  for (k in 1:length(descriptionPos)) {
    if (((descriptionPos[k] + 1) == (endPos[k] - 1))) {
      ...
      if ((endPos[k] - 1) > (descriptionPos[k] + 1)) {
        textout <- paste(
          docu[((descriptionPos[k] + 1):(endPos[k] - 1))],
          collapse = " "
        )
      }
    }
  ...
  modelCrosswalk <- data.frame(
    ...
    MaxTreeSizeClass = sapply(maxTree, get.maxTreeSizeClass),
...
crosswalk$Model_Code <- sapply(crosswalk$Model_Code, function(x) {
  ifelse(
    substr(x, start = nchar(x), stop = nchar(x)) == "_",
...
# Structural stage
crosswalk$StructuralStage <- sapply(crosswalk$StructuralStage, function(x) {
  ifelse(
    x %in%
      c("All Structures", "All", " All"),
    "ALL",
    ifelse(x == "Closed", "CLS", "OPN")
  )
})
